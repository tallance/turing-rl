...
- Pull run results (plots, reports, metrics) locally into `results/<plan-name>/` (plan filename without `.md`); include a `README.txt` with repro commands + cluster source paths.
- **Multi-agent repo (shared branch + cluster checkout).** Other agents work here concurrently — make **additive commits only**. `git reset`/`rebase`/force-push, or reverting/deleting another agent's commits or files, needs my explicit permission first: flag the justification, don't act on your own judgment. You own only the files you edit; disjoint files = just proceed, same-file collisions = stop and flag. Before `sync_to_cluster.sh`, ensure your HEAD still contains others' commits so a deploy never drops their code. If it looks like another agent violated this (your commits/files vanished, history was rewritten, or HEAD moved unexpectedly), stop and report it to me immediately.
