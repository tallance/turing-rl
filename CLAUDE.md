...
- **Multi-agent repo (shared branch + cluster checkout).** Other agents work here at the same time. Make **additive commits only**: never `git reset`/`rebase`/force-push the shared branch, and never revert, drop, or delete another agent's commits or files — even if they look like scope-creep, they aren't yours to prune. You only own the files you actually edit. If two agents genuinely touch the **same files** (real collision), stop and flag it to me — don't resolve it unilaterally. Disjoint files = just proceed. Before `sync_to_cluster.sh`, make sure your HEAD still contains others' commits so a deploy never drops their code.
