...
| Served-eval max completion tokens | `PERSONA_JUDGE_MAX_COMPLETION_TOKENS=8192` | HTTP judge evaluation default; inference and RL training need not share a token budget |
| Judge-RL max response tokens | `data.max_response_length=10752` with `actor_rollout_ref.model.use_fused_kernels=true` | SSOT `training/grpo/configs/qwen35_judge_grpo.yaml`; the fused loss avoids materializing full-vocabulary logits per token |
| **In-training validation sampling** | **`temperature=0.6`, `top_p=0.8`, `top_k=20`, `n=1`** | SSOT `training/grpo/configs/qwen35_judge_grpo.yaml` → `rollout.val_kwargs`. This is a **separate mechanism** from `PERSONA_JUDGE_SAMPLING` above, which only reaches the judge served over HTTP — it does not reach the trainer's own validation rollouts. Keep the temperatures equal; the trainer's validation inherits its 10,752-token training cap while served eval remains capped at 8,192. No `repetition_penalty` here yet (2026-08-25) |
