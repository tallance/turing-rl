...
| 2 | `eval/single_token_judge.py` (new) + `scripts/run_judge_sweep_cell.py` (dispatch) | Dispatch on `JUDGE_PROMPT_STYLE` (`full` default, `single_token`) where both eval launchers converge. Single-token path: `max_completion_tokens=1`, no `response_format`, `logprobs=True`, `top_logprobs=20`, `enable_thinking` pinned off. Emits the same per-call dump shape as the reward arm plus `letter`/`p_a`/`ab_mass`, under an extra `<style>` path segment. (Originally specified as a branch inside `eval/metrics.py`; that module is on neither eval pipeline's path.) |
